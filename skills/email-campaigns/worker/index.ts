	...
	async fetch(req: Request, env: Env): Promise<Response> {
		...
		if (url.pathname === '/newsletter/welcome' && req.method === 'POST') {
			try {
				...
				await sendBrevoEmail(env, {
					...
					subject: `Bienvenido/a a ${BRAND.name} — gracias por suscribirte`,
					htmlContent: html,
				...
				try {
					...
					await sendBrevoEmail(env, {
						...
						subject: `[${BRAND.name}] Nuevo suscriptor: ${email}`,
		...
		if (url.pathname === '/unsubscribe' && req.method === 'GET') {
			...
			return unsubscribeResultPage(
				true,
				email,
				`You won't receive marketing emails from ${BRAND.name}. You can resubscribe anytime by replying.`,
			);
		}
...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;background:#ffffff;border:1px solid #e7e5e4;border-radius:8px;">
  <tr><td style="padding:20px 24px;border-bottom:1px solid #e7e5e4;">
    <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:#a8a29e;">${BRAND.name} admin · newsletter</div>
  ...
  <tr><td style="padding:20px 24px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="font-size:13px;color:#44403c;">
      ...
      <tr><td style="padding:4px 0;color:#78716c;">Hora</td><td style="padding:4px 0;">${escapeHtml(when)}</td></tr>
    </table>
  </td></tr>
  <tr><td style="padding:0 24px 24px;">
    <a href="${BRAND.siteUrl}${BRAND.adminPathContacts}" style="display:inline-block;padding:10px 16px;background:#1c1917;color:#ffffff;border-radius:6px;font-size:13px;font-weight:500;text-decoration:none;">Ver en ${BRAND.adminPathContacts}</a>
  </td></tr>
</table></td></tr></table>
</body></html>`;
}
    ...
    <tr><td align="center">
      <table role="presentation" width="560" style="max-width:560px;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.06);">
        ...
        <tr><td style="padding:32px 40px 8px;">
          ...
          <p style="margin:14px 0 0;color:#666;font-size:15px;line-height:1.65;">Ya estás en la lista de ${BRAND.name}. Te escribiremos de vez en cuando — sin spam, sin prisa.</p>
        </td></tr>
        <tr><td style="padding:28px 40px 8px;" align="center">
          <a href="${BRAND.siteUrl}" style="display:inline-block;background:#1a1a1a;color:#ffffff;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:600;font-size:15px;letter-spacing:0.3px;">Explorar ${BRAND.name}</a>
        </td></tr>
        ...
        <tr><td style="padding:24px 40px 28px;text-align:center;">
          ...
          <div style="font-size:11px;color:#aaa;margin-top:16px;">
            <a href="${unsubUrl}" style="color:#aaa;text-decoration:underline;">Cancelar suscripción</a> · <a href="${BRAND.siteUrl}" style="color:#aaa;text-decoration:none;">${BRAND.domain}</a>
          </div>
