...
import { ArrowRight, Utensils, Calendar, MapPin } from 'lucide-react';
...
const EnjoyCocinaPage = () => {
  const { t, i18n } = useTranslation('enjoyCocina');
  ...
  return (
    <>
      ...
      <main id="main" role="main" className="bg-background text-foreground">
        ...
        <section className="py-24 px-4 sm:px-6 lg:px-8 bg-background">
          <m.div {...fadeIn} className="max-w-3xl mx-auto">
            <div className="flex items-center gap-2 text-[#D4A574] text-sm uppercase tracking-[0.25em] mb-6">
              ...
              <span>Cocina caribeña · Casa Club</span>
            </div>
        ...
        {/* Gallery — single feature shot. Mobile: full-bleed edge to
            edge. Desktop: centered, larger (max-w-4xl) with rounded
            corners so it reads as a presented frame. */}
        <section className="py-16 bg-card">
          <m.h2 {...fadeIn} className="text-2xl sm:text-3xl font-serif text-foreground mb-10 text-center px-4">
            {t('galleryHeading')}
          </m.h2>
          <div className="max-w-4xl mx-auto sm:px-6 lg:px-0">
            {GALLERY.map((src, i) => (
              <m.div
                key={src}
                ...
                className="overflow-hidden rounded-none sm:rounded-2xl shadow-xl"
              >
                <img
                  src={src}
...
export default EnjoyCocinaPage;
