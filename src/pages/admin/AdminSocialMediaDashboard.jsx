...
const AdminSocialMediaDashboard = () => {
  const { t } = useTranslation('adminSocialDashboard');
  ...
  const handleRetry = async (platform, socialPostId, blogPostId) => {
    toast({ title: t('toast.retrying'), description: t('toast.retryingDesc', { platform }) });
    try {
      ...
      toast({ title: t('toast.success'), description: t('toast.retrySuccess', { platform }) });
      fetchData();
    } catch (err) {
      toast({ variant: 'destructive', title: t('toast.retryFailed'), description: err.message });
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="flex justify-between items-end">
            ...
            <Button onClick={fetchData} variant="outline" className="gap-2">
              <RefreshCw className={loading ? "animate-spin w-4 h-4" : "w-4 h-4"} /> {t('buttons.refresh')}
            </Button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
              <h3 className="text-sm font-medium text-stone-500">{t('metrics.totalPosts')}</h3>
              <p className="text-3xl font-semibold text-stone-900 mt-2">{totalCreated}</p>
            </div>
            <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
              <h3 className="text-sm font-medium text-stone-500">{t('metrics.successRate')}</h3>
              <p className="text-3xl font-semibold text-stone-900 mt-2">{successRate}%</p>
            </div>
            <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
              <h3 className="text-sm font-medium text-stone-500">{t('metrics.failedPosts')}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden flex flex-col">
              <div className="p-6 border-b border-stone-100 bg-stone-50">
                <h2 className="text-lg font-medium text-stone-900">{t('sections.recentSocial')}</h2>
              </div>
              <div className="overflow-x-auto p-0">
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-stone-500 uppercase bg-stone-50 border-b">
                    <tr>
                      <th className="px-6 py-3">{t('tableSocial.platform')}</th>
                      <th className="px-6 py-3">{t('tableSocial.status')}</th>
                      <th className="px-6 py-3">{t('tableSocial.date')}</th>
                      <th className="px-6 py-3 text-right">{t('tableSocial.action')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-stone-100">
                    {loading ? (
                      ...
                      <tr><td colSpan="4" className="text-center py-8 text-stone-500">{t('tableSocial.empty')}</td></tr>
                      ...
                      <tr key={post.id} className="hover:bg-stone-50">
                        ...
                        <td className="px-6 py-4">
                          {post.status === 'posted' ? <Badge variant="default" className="bg-green-100 text-green-800 hover:bg-green-100"><CheckCircle2 className="w-3 h-3 mr-1"/> {t('status.posted')}</Badge> :
                           post.status === 'failed' ? <Badge variant="destructive"><XCircle className="w-3 h-3 mr-1"/> {t('status.failed')}</Badge> :
                           <Badge variant="outline" className="text-stone-500">{t('status.pending')}</Badge>}
                        </td>
            ...
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden flex flex-col">
              <div className="p-6 border-b border-stone-100 bg-stone-50">
                <h2 className="text-lg font-medium text-stone-900">{t('sections.recentBlogs')}</h2>
              </div>
              <div className="overflow-x-auto p-0">
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-stone-500 uppercase bg-stone-50 border-b">
                    <tr>
                      <th className="px-6 py-3">{t('tableBlogs.title')}</th>
                      <th className="px-6 py-3">{t('tableBlogs.status')}</th>
                      <th className="px-6 py-3">{t('tableBlogs.keywords')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-stone-100">
                    {loading ? (
                      ...
                      <tr><td colSpan="3" className="text-center py-8 text-stone-500">{t('tableBlogs.empty')}</td></tr>
                    ) : blogPosts.map(post => (
                      <tr key={post.id} className="hover:bg-stone-50">
                        <td className="px-6 py-4 font-medium text-stone-900 truncate max-w-[200px]">{post.title}</td>
                        <td className="px-6 py-4">
                          {post.published ?
                            <Badge variant="outline" className="border-green-200 text-green-700 bg-green-50">{t('status.published')}</Badge> :
